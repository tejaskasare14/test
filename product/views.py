...
def filter_by_category(request,category_value):
   ...
   global filterd_products
   if(category_value=="0"):
      filterd_products=ProductTable.objects.filter(is_active=True)
      data['products']=filterd_products
   else:
      filterd_products=products.filter(q1 & q2)
      data['products']=filterd_products 
   return render(request,'product/index.html',context=data)
...
def sort_by_price(request,sort_value):
   ...
   if sort_value=='asc':
      ...
      price = '-price'
   global filterd_products
   print("filterd_products",filterd_products)
   sorted_products=filterd_products.filter(is_active=True).order_by(price)
   data['products']=sorted_products
   return render(request,'product/index.html',context=data)
#---------------------------------------------------
...
def filter_by_rating(request,rating_value):
   ...
   q2 = Q(rating__gt=rating_value)
   global filterd_products
   rating_filterd_products=filterd_products.filter(q1 & q2)
   data['products']=rating_filterd_products
   return render(request,'product/index.html',context=data)
...
def filter_by_price_range(request):
   ...
   q3 = Q(is_active=True)
   global filterd_products
   price_filterd_products=filterd_products.filter(q1 & q2 & q3)
   data['products']=price_filterd_products
   return render(request,'product/index.html',context=data)
